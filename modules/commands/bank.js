...
module.exports.run = async function({ api, event, args, Users, Currencies }) {
    ...
    const moneyFormat = amount => amount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    ...
    if (!command || command === "help") {
        return sendMessage(
            ...
            "3. /bank gửi [số tiền] - Gửi tiền vào tài khoản\n" +
            "4. /bank rút [số tiền] - Rút tiền từ tài khoản\n" +
    ...
    if (command === "info") {
        ...
        return sendMessage(
            ...
            `💰 Số dư ngân hàng: ${moneyFormat(accountInfo.balance)} đồng\n` +
            `💵 Số dư ví: ${moneyFormat(walletBalance)} đồng\n` +
    ...
    if (command === "gửi") {
        ...
        if (isNaN(amount) || amount <= 0) {
            return sendMessage("❌ Vui lòng nhập số tiền hợp lệ để gửi!");
        }
        
        const walletBalance = await getUserBalance(senderID);
        if (walletBalance < amount) {
            return sendMessage(`❌ Số dư ví của bạn không đủ để gửi ${moneyFormat(amount)} đồng!`);
        }
        ...
        return sendMessage(
            `✅ Gửi tiền thành công!\n\n` +
            `💰 Số tiền: ${moneyFormat(amount)} đồng\n` +
            `💼 Số dư ngân hàng mới: ${moneyFormat(bankData[senderID].balance)} đồng\n` +
            `⏰ Thời gian: ${getTime()}`
        );
    }
    ...
    if (command === "rút") {
        ...
        const amount = parseInt(args[1]);
        if (isNaN(amount) || amount <= 0) {
            return sendMessage("❌ Vui lòng nhập số tiền hợp lệ để rút!");
        }
        
        if (bankData[senderID].balance < amount) {
            return sendMessage(`❌ Số dư tài khoản của bạn không đủ để rút ${moneyFormat(amount)} đồng!`);
        }
    ...
    if (command === "chuyển") {
        ...
        if (isNaN(amount) || amount <= 0) {
            return sendMessage("❌ Vui lòng nhập số tiền hợp lệ để chuyển!");
        }
        ...
        if (bankData[senderID].balance < amount) {
            return sendMessage(`❌ Số dư tài khoản của bạn không đủ để chuyển ${moneyFormat(amount)} đồng!`);
        }
        ...
        api.sendMessage(
            ...
            `💰 Số tiền: ${moneyFormat(amount)} đồng\n` +
            `📝 Nội dung: ${content}\n` +
            `💼 Số dư ngân hàng mới: ${moneyFormat(bankData[receiverID].balance)} đồng\n` +
            `⏰ Thời gian: ${getTime()}`,
            receiverID
        );
    ...
    if (command === "history") {
        ...
        for (let i = transactions.length - 1; i >= 0; i--) {
            ...
            msg += `${transactions.length - i}. [${t.time}] ${typeText}: ${moneyFormat(t.amount)} đồng\n`;
            msg += `   📝 ${t.description}\n\n`;
        }
