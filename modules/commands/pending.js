  ...
  const request = require('request');
    ...
    if (isNaN(body) && body.indexOf("c") == 0 || body.indexOf("cancel") == 0) {
        ...
        for (const singleIndex of index) {
            ...
            api.changeNickname(`『 ${global.config.PREFIX} 』 ⪼ ${(!global.config.BOTNAME) ? "MiraiV3👾" : global.config.BOTNAME}`, handleReply.pending[singleIndex - 1].threadID, api.getCurrentUserID());
            api.sendMessage("", event.threadID, () => api.sendMessage(`❯ Admin Bot: ${global.config.NDH}`, handleReply.pending[singleIndex - 1].threadID));
            count+=1;
